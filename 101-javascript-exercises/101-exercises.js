...
function endsWithVowel(word) {
    ...
    for (var i = 0; i < str.length; i++) {
        if (last == 'a' || last == 'e' || last == 'i' || last == 'o' || last == 'u' || last == 'A' || last == 'E' || last == 'O' || last == 'U')
            break;
    }
    if (i == 0) {
        return true;
...
function startsAndEndsWithVowel(word) {
    // if (startsWithVowel(word) == true && endsWithVowel(word) == true) {
    if (startsWithVowel(word) && endsWithVowel(word) === true)
        return true
    else return false
}
...
function thirdToLast(input) {
    if (input == 'string') {
        return input.split('')[input.length - 3];
    } else {
        if (Array.isArray(input) === true) ;
...
// first remove first element from array
    var b = arr.shift();
    arr[arr.length] = b;
    return arr;
}
...
function mean(arr) {
    const avg = sumAll(arr) / arr.length;
    return avg;
}
...
//that takes in sequence of numbers and returns the average value
...
function maxMinusMin(arr) {
    var maxSubMin = Math.max(...arr) - Math.min(...arr);
    return maxSubMin;
}
...
function shortestString(arr) {
    ...
    arr.map(v => min = Math.min(min, v.length));
    var newArray = arr.filter(v => v.length == min);
    return newArray[0];
}
...
function longestString(arr) {
    ...
    var newArray = arr.filter(i => i.length == max);
    return newArray[0];
}
...
function totalOfBookPrices(books) {
    const sumTotal = books.reduce((prev, cur) => prev + cur.price, 0);
    return sumTotal;
}
